...
describe('PUT /api/signatures', function () {
	...
	it('using invalid passphrase should fail', function (done) {
		...
		putSignature(validParams, function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.not.ok;
			node.expect(res.body).to.have.property('error').to.match(/Mnemonic string is invalid/);
			done();
		});
	});
	...
	it('using valid parameters should be ok', function (done) {
		putSignature(validParams, function (err, res) {
			...
			node.expect(res.body.transaction).to.have.property('senderPublicKey').to.equal(account.publicKey.toString('hex'));
